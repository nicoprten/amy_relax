...
export const ProfileMenu = ({ user }) => {
    ...
    return(
        <div className='relative'>
            {/* NAVBAR */}
            <div className={showProfileCard ? 'w-max flex gap-2 justify-items-end items-center p-1 rounded-full text-blue border-1 border-black bg-white hover:cursor-pointer' : 'w-max flex gap-2 justify-items-end items-center p-1 rounded-full text-black border-1 hover:cursor-pointer hover:bg-white hover:text-blue duration-200'} onClick={() => setShowProfileCard(!showProfileCard)}>
                <img className='w-[40px] rounded-full' src={user ? user.image : ''} alt={`Foto de perfil de ${user?.name}`} />
                {/* <p className='text-sm'>{user?.name}</p> */}
            </div>
            {/* MENU PROFILE */}
            {showProfileCard &&
                <div className='flex flex-col items-center gap-4 absolute top-[60px] right-[-20px] bg-brown-dark px-2 py-4 rounded shadow-xl border-1 border-black md:w-[200px]'>
                    ...
                    <p className='text-xs text-blue'>{user?.email}</p>
