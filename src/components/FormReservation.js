...
export const FormReservation = ({error, setError}) => {
            ...
            <div className='relative sm:grow bg-white0 border-1 border-gray rounded p-4 pb-8 shadow-lg text-sm'>
                ...
                <div className="flex gap-2 flex-wrap mt-4">
                    ...
                    <div className='relative border-1 border-gray p-2 m-2 w-full group md:w-max'>
                        ...
                        <input className='p-2 focus:outline-0 w-full md:w-max' id='address' type="text" value={user.address} onChange={(e) => dispatch(changeUser({address: e.target.value}))}></input>
                        <span className='z-10 absolute left-[-1px] w-max bg-black px-2 rounded text-xs text-brown hidden group-hover:block duration-200'>To know where to go</span>
                    </div>
                </div>
                {error && 
